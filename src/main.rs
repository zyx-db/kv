...
fn handle_request(request: Request, kv_store: Arc<DB>) {
...
fn handle_get(request: Request, kv_store: Arc<DB>) {
    let path = request.url();
    ...
    if let Some(value) = kv_store.get(key) {
        ...
        let response = Response::from_string("Key not found")
            .with_status_code(404);
        request.respond(response).unwrap();
    }
}

fn handle_set(mut request: Request, kv_store: Arc<DB>) {
    ...
    if parts.len() == 2 {
        ...
        let response = Response::from_string("Invalid input")
